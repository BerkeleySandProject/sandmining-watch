...
def print_trainable_parameters(model):
    ...
    print(
        f"trainable params: {trainable_params/1e6}M || all params: {all_param/1e6}M || trainable%: {100 * trainable_params / all_param:.2f}"
    )


def model_factory(
    config: Union[
        SupervisedTrainingConfig, SupervisedFinetuningConfig, ThreeClassConfig
    ],
    n_channels,
    config_lora=None,
):
    if isinstance(config, ThreeClassConfig) and (
        ...
        n_classes = 1
    if (
        isinstance(config, SupervisedFinetuningConfig)
        and config.num_upsampling_layers is None
    ):
    ...
    if config.model_type == ModelChoice.UnetSmall:
        ...
        segformer_config = SegformerConfig(
            num_channels=n_channels, num_labels=n_classes
        )  # SegFormer-B0
        model = SegformerForSemanticSegmentationForRV(
            segformer_config, img_size=config.tile_size
        )
    elif config.model_type == ModelChoice.SatmaeBaseLinearDecoder:
        from models.satmae.satmae_encoder_custom_decoder.satmae_encoder_linear_decoder import (
            SatMaeSegmenterWithLinearDecoder,
        )

        model = SatMaeSegmenterWithLinearDecoder(
            "base", image_size=config.tile_size, num_classes=n_classes
        )
    elif config.model_type == ModelChoice.SatmaeBaseDoubleUpsampling:
        from models.satmae.satmae_encoder_custom_decoder.satmae_encoder_double_upsampling import (
            SatMaeSegmenterWithDoubleUpsampling,
        )
    ...
    if (
        isinstance(config, SupervisedFinetuningConfig)
        and not isinstance(config, InferenceConfig)
        and config.finetuning_strategy == FinetuningStratagyChoice.LoRA
    ):
