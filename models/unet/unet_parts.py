...
class SmallUp(nn.Module):
    ...
    def __init__(self, up_in_channels, skip_in_channels, mid_channels, out_channels, bilinear=True):
        super().__init__()

        self.skip_conv = nn.Sequential(
            nn.Conv2d(skip_in_channels, mid_channels // 2, kernel_size=3, padding=1), 
        ...
        if bilinear:
            self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.conv = DoubleConv(mid_channels, out_channels, mid_channels // 2)
        else:
            self.up = nn.ConvTranspose2d(up_in_channels, mid_channels // 2, kernel_size=2, stride=2)
