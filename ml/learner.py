...
from ml.semantic_segmentation_labels import (
    SemanticSegmentationSmoothLabelsCustom,
    SemanticSegmentationLabelsCustom,
)
...
from experiment_configs.schemas import (
    ...
    BackpropLossChoice,
    FinetuningStratagyChoice,
    ThreeClassConfig,
    ThreeClassSupervisedTrainingConfig,
    ThreeClassVariants,
)
from ml.losses import DiceLoss, WeightedBCE, WeightedDiceLoss
from ml.model_stats import count_number_of_weights
from utils.wandb_utils import (
    create_semantic_segmentation_image,
    create_predicted_probabilities_image,
)
...
class Learner(ABC):
    ...
    def __init__(
        ...
        if save_model_checkpoints:
            ...
            self.model_checkpoints_dir = (
                f"/data/sand_mining/training_checkpoints/{outdir_dir_folder_name}"
            )
            make_dir(self.model_checkpoints_dir)
            print(
                f"Will save weights after every epoch to {self.model_checkpoints_dir}"
            )
        else:
    ...
    def wrapup(self):
        ...
        if (
            isinstance(self.config, SupervisedFinetuningConfig)
            and self.config.finetuning_strategy == FinetuningStratagyChoice.LoRA
        ):
    ...
    def build_dataloaders(self) -> Tuple[DataLoader, DataLoader, DataLoader]:
        ...
        train_dl = DataLoader(
            ...
            worker_init_fn=lambda x: torch.multiprocessing.set_sharing_strategy(
                "file_system"
            )
    ...
    def calculate_losses(self, out, y, prefix=""):
        return {
            f"{prefix}{loss_fn_name}": loss_fn(out, y)
    ...
    def plot_dataloaders(self, batch_limit: Optional[int] = None, show: bool = False):
    ...
    def train_epoch(
        self, optimizer: "Optimizer", step_scheduler: Optional["_LRScheduler"] = None
    ) -> MetricDict:
    ...
    def save_model_weights(self, path: str, save_lora=True):
        ...
        if (
            save_lora
            and isinstance(self.config, SupervisedFinetuningConfig)
            and self.config.finetuning_strategy == FinetuningStratagyChoice.LoRA
        ):
    ...
    def on_epoch_end(self, curr_epoch, metrics):
        ...
        if self.model_checkpoints_dir:
            torch.save(
                ...
                f"{self.model_checkpoints_dir}/after_epoch_{curr_epoch}.pth",
            )
    ...
    def metrics_to_log_wand(self, metrics):
        ...
        if self.step_scheduler is not None:
            metrics_to_log["lr_at_epoch_end"] = get_schedulers_last_lr(
                self.step_scheduler
            )
...
class BinarySegmentationLearner(Learner):
    def build_loss_functions(self) -> dict:
        if (
            isinstance(self.config, ThreeClassConfig)
            and self.config.three_class_training_method == ThreeClassVariants.A
        ):
            return {
                "bce_loss": WeightedBCE(
                    nonmine_weight=1.0, mine_weight=self.config.mine_class_loss_weight
                ),
    ...
    def validate_end(self, outputs, num_samples):
        ...
        if (
            ...
            # Filtering out low confidence
            out_probabilities = out_probabilities[mask]
            ground_truths = ground_truths[mask]  # ""
            # Assigning high confidence to value 1 to work with binary outputs
        ...
        _, _, _, average_precision, best_threshold, best_f1_score = compute_metrics(
            ground_truths.cpu().numpy(), out_probabilities.cpu().numpy()
        )
...
class MultiSegmentationLearner(Learner):
    ...
    def validate_end(self, outputs, num_samples):
        ...
        out_classes = self.prob_to_pred(
            out_probabilities.view(
                num_samples, 3, self.config.tile_size, self.config.tile_size
            )
        ).reshape(-1)
        conf_mat = compute_conf_mat(
            out_classes,
        ...
        high_conf_probs = out_probabilities.reshape(
            num_samples, 3, self.config.tile_size, self.config.tile_size
        )[:, 2, :, :]  # Select only high confidence sandmine
        high_conf_probs = high_conf_probs.reshape(-1)
        mask = ground_truths != 1
        # Filtering out low confidence
        high_conf_probs = high_conf_probs[mask]
...
class Predictor(ABC):
    ...
    def predict_dataloader(
        ...
        preds = self._predict_dataloader(
            dl, raw_out=raw_out, batched_output=batched_output, predict_kw=predict_kw
        )
    ...
    def predict_site(
        ...
        predictions_on_dl = (
            self.predict_dataset(  # Each 160x160 (or other sized) images are predicted
                ds,
                numpy_out=True,
                progress_bar=False,
            )
...
class BinarySegmentationPredictor(Predictor):
    ...
    def prob_to_pred(self, out, threshold=0.5):
...
class MultiSegmentationPredictor(Predictor):
    ...
    def predict_site(
        ...
        return super().predict_site(ds, crop_sz, num_classes=3)
