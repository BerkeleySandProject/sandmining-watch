...
class MultiClassSegmentationLearner(ABC):
    """
    Training routines for a model.
    ...
    def __init__(self,
        ...
        # TODO: Replace with other loss functions
        # self.bce_loss = BCEWithConfidence()
        # self.dice_loss = DiceLoss()
        self.ce_loss = nn.CrossEntropyLoss(weight=self.to_device(
            torch.tensor([self.config.nonmine_class_weight, self.config.uncertain_class_weight, 1.]), self.device))
    ...
    def train_step(self, batch, batch_ind):
        ...
        out = self.post_forward(self.model(x))
        # In the following, we hardcoded our metric names for our loss functions
        return {"train_ce_loss": self.ce_loss(out, y),}
 
    def validate_step(self, batch, batch_ind):
        x, y = batch
        out = self.post_forward(self.model(x))
        val_ce_loss = self.ce_loss(out, y)
        out_probabilities = torch.softmax(out, dim=1)
    ...
    def validate_end(self, outputs, num_samples):
        ...
        out_classes =  self.prob_to_pred(out_probabilites).view(-1)
        conf_mat = compute_conf_mat(out_classes, ground_truths, 3)
        conf_mat_metrics = compute_conf_mat_metrics(conf_mat, self.class_names)

        mcap = MulticlassAveragePrecision(thresholds=None, num_classes=3)
        average_precision = mcap(out_probabilites, ground_truths)

        metrics = {
            **conf_mat_metrics,
            "sandmine_average_precision": average_precision,
            "val_ce_loss": val_ce_losses.sum() / num_samples,
        }
        return metrics
    ...
    def train_epoch(
        ...
        with tqdm(self.train_dl, desc='Training') as bar:
            for batch_ind, (x, y) in enumerate(bar):
                ...
                if self.config.loss_fn == BackpropLossChoice.CE:
                    loss_to_backpropagate = output["train_ce_loss"]
                else:
                    raise ValueError(f"Unexpected value for loss_fn")
                loss_to_backpropagate.backward()
