    ...
    def train_epoch(
        ...
        with tqdm(self.train_dl, desc='Training') as bar:
            for batch_ind, (x, y) in enumerate(bar):
                ...
                if isinstance(self.config, ThreeClassSupervisedTrainingConfig):
...
class MultiSegmentationLearner(ABC):
    ...
    def __init__(self,
                 config: ThreeClassSupervisedTrainingConfig,
                 model: nn.Module,
                 train_ds: 'Dataset',
                 valid_ds: 'Dataset',
                 output_dir: Optional[str] = None,
    ...
    def train_step(self, batch, batch_ind):
        x, y = batch
        output = self.post_forward(self.model(x))
        # In the following, we hardcoded our metric names for our loss functions
        return {"train_ce_loss": self.loss(output, y),}
...
class MultiSegmentationPredictor(ABC):
    """
    Prediction routines for a model.
    """
